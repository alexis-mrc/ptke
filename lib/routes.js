...
module.exports = ({ cache, config }) => {
  const { loadCache, getAllReleases } = cache
  const exports = {}
  ...
  exports.releaseNotes = async (req, res) => {
    const releases = await getAllReleases()

    if (!releases || releases.length === 0) {
      send(res, 404, { error: 'No release notes available' })
      return
    }
  ...
  exports.latestVersion = async (req, res) => {
    ...
    if (!latest.version) {
      send(res, 404, { error: 'No version information available' })
      return
    }

    const platforms = latest.platforms || {}

    // Build the response matching the VersionInfo interface
    const response = {
      version: latest.version,
      releaseDate: latest.pub_date,
      mac: platforms.dmg ? {
        url: platforms.dmg.url,
        size: String(platforms.dmg.size || 0)
      } : undefined,
      windows: platforms.exe ? {
        url: platforms.exe.url,
        size: String(platforms.exe.size || 0)
      } : undefined
    }

    // Remove undefined platforms
    if (!response.mac) delete response.mac
    if (!response.windows) delete response.windows
